...
describe("getGenres", () => {
  it("Возвращает первые два жанра", () => {
    expect(
      getGenres({
        genres: [{ name: "драма" }, { name: "ужасы" }, { name: "комедия" }],
      } as MovieDetails),
    ).toBe("драма, ужасы");
  });
...
describe("getDurationInfo", () => {
  ...
  it("Возвращает количество сезонов", () => {
    expect(getDurationInfo({ original_name: "Ходячие мертвецы", number_of_seasons: 11 })).toBe(
      "11 сез.",
    );
  });
...
describe("Возвращает getPosterUrl, getBackUrl, getBackdropUrl - url", () => {
  ...
  it.each([getPosterUrl, getBackUrl, getBackdropUrl])("возвращает заглушку", (fn) => {
